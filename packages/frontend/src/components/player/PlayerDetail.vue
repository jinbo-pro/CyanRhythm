...
const coverSrc = computed(() => {
  ...
  if (player.currentSong.cover) return player.currentSong.cover
  return player.currentSong.hasCover ? coverUrl(player.currentSong.fileRelPath) : ''
})
...
// 歌曲切换：更新封面并重新连接分析器（新 Howl 产生新 audio 元素）
watch(
  () => player.currentSong?.id,
  () => {
    if (visualizer) visualizer.updateCover(coverSrc.value)
...
<template>
  <div class="fixed inset-0 z-[2000] flex flex-col overflow-hidden">
    ...
    <main class="relative z-[2] flex min-h-0 flex-1">
      ...
      >
        <div class="flex flex-1 flex-col items-center justify-center gap-2 p-10">
          <el-icon :size="56" class="mb-3 text-white/20"><Reading /></el-icon>
          <p class="text-lg font-medium text-white/50">歌词功能开发中</p>
          <p class="text-[13px] text-white/30">我们后续会开发歌词部分</p>
        </div>
      </aside>
    </main>
