...
/** 读取已扫描目录列表 */
export async function loadScanDirs() {
  const db = await getDB()
  return (await db.get('library', SCAN_DIRS_KEY)) || []
}

/** 保存已扫描目录列表 */
export async function saveScanDirs(dirs) {
  const db = await getDB()
  await db.put('library', dirs, SCAN_DIRS_KEY)
}
